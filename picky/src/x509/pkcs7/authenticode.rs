...
use picky_asn1::wrapper::{ApplicationTag0, ApplicationTag1, Asn1SetOf, ObjectIdentifierAsn1};
...
use picky_asn1_x509::{oids, Attribute, AttributeValues, Attributes, Certificate, DigestInfo, Name, ShaVariant};
...
impl AuthenticodeSignature {
    pub fn new(
        ...
        let program_name = program_name.map(SpcString::try_from).transpose()?.map(ApplicationTag0);
