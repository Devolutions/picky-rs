...
// (`JwsAlg::EdDSA` should be used instead) but we should keep it inside enum to allow decoding of
// invalid tokens generated by other libraries such as `golang-jws`. However, to avoid compilation
// warnings, we need to allow deprecated items inside this module.
#![allow(deprecated)]
