...
//! PKCS8 private keys. Encryption on `safe bag` level is performed to allow PFX files which have
...
//! which is set to same for both certificate and private key.
//!
//! # Parsing
//! Almost any kind of PKCS#12 object specified by [RFC](https://datatracker.ietf.org/doc/html/rfc7292)
//! could be parsed or constructed. However, only most used PFX types are wrapped in high level
...
//! via `.inner()` method which usually will return ASN.1 DER encoded data (`picky-asn1-x509` crate types).
//!
//! Parsing process could be controlled even more with [`Pkcs12ParsingParams`], which allows to skip
//! some parsing errors (e.g. Skip failed mapping of private key ASN.1 structure to high level picky wrapper)
//!
...
//! bags could be inspected and data could be extracted from PFX.
//!
//! # Building
//! Building process of PFX files is down-to-top, in contrast to parsing. It strats with
//! building of required safe bags/attributes structures, wrapping them inside safe contentents
...
//! PFX files with Pbes1 encryption is to support legacy software which does not support Pbes2.
//! - Usually PFX files without password are actually encrypted with empty-string password if
//! generated by modern software by default. (e.g. certmgr or OpenSSL). However it is possible to
//! create PFX files without any ecnryption and MAC in picky if PFX will be used just as a plain
